...
class Am_Paysystem_Billplz extends Am_Paysystem_Abstract {
    ...
    public function _initSetupForm(Am_Form_Setup $form) {
        ...
        $form->addText('productionsandbox', array('size' => 2))
                ...
                ->addRule('required');
        $form->addText('billplzautosubmit', array('size' => 2))
                ->setLabel('Leave Blank or "FPX" or "PayPal"');
    ...
    public function _process(Invoice $invoice, Am_Mvc_Request $request, Am_Paysystem_Result $result) {
        $Payment_Method = '';
        $api_key = $this->getConfig('merchant_id') == '' ? 'api-key' : $this->getConfig('merchant_id');
        $collection_id = $this->getConfig('verify_key') == '' ? 'collection-id' : $this->getConfig('verify_key');
        $host = DapatkanHost($this->getConfig('productionsandbox'));
        $auto_submit = strtolower($this->getConfig('billplzautosubmit'));
        $deliver = false;
        ...
        $url = $arr['url'].'?auto_submit='.$auto_submit;
